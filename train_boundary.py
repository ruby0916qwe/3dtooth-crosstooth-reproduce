...
def get_args():
    ...
    parser.add_argument('--train_dir', type=str, default="b/data_144/train")
    parser.add_argument('--val_dir', type=str, default="b/data_144/test")
...
def evaluate(model, dataloader, args, num_classes=2):
    ...
    with torch.no_grad():
        for pointcloud, labels, _ in dataloader:
            ...
            face_coordinate = pointcloud[:, :3, :].permute(0, 2, 1).cpu().numpy()  # (B, N, 3)

            for i in range(batch_size):
                sample_pred_logits = edge_seg_result[i].unsqueeze(0)  # (1, 2, N)
                face_centers = face_coordinate[i]                            # (N, 3)
...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, (pointcloud, labels, face_info) in enumerate(train_loader):
            ...
            for i in range(batch_size):
                face_centers = face_coordinate[i]          
                label_np = labels[i].cpu().numpy()  # (N,)
                
                boundary_mask = compute_boundary_mask(face_centers, label_np, threshold=args.threshold, k=8)  # bool (N,)
