...
def batch_transform_predict():
    """
    对test文件夹中的每个数据集运行transform_predict
    输出保存到test_for_predict文件夹
    """
    # 源文件夹和目标文件夹
    source_dir = "test"
    target_dir = "test_for_predict"
    ...
    for i, pt_file in enumerate(pt_files, 1):
        ...
        # 构建输出文件名
        output_file = f"{base_name}_for_predict.ply"
        
        # 构建完整路径
        input_path = os.path.join(source_dir, pt_file)
        output_path = os.path.join(target_dir, output_file)
        
        print(f"[{i}/{len(pt_files)}] 处理: {pt_file} -> {output_file}")
        
        try:
            # 运行transform_predict.py
            result = subprocess.run([
                sys.executable, 'transform_predict.py', 
