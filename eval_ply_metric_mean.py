...
def calculate_miou(gt_labels, pred_labels, n_class):
    ...
    for c in range(1, n_class):  # 排除背景0
        gt_mask = (gt_labels == c).astype(np.int64)
        pred_mask = (pred_labels == c).astype(np.int64)
        gt_mask_torch = torch.from_numpy(gt_mask)
        pred_mask_onehot = torch.from_numpy(np.eye(2)[pred_mask]).float()
        _, _, _, _, iou, _ = cal_metric(gt_labels=gt_mask_torch, pred_labels=pred_mask_onehot, target_class=1)
        ious.append(iou)
        weights.append(gt_mask.sum())
    valid_ious = [iou for iou in ious if iou >= 0.001]
    if len(valid_ious) > 0:
        mean_miou = np.mean(valid_ious)
    else:
        mean_miou = 0.0
    return mean_miou, ious, weights
...
def calculate_boundary_miou(gt_centers, gt_labels, pred_labels, n_class, k=8):
    ...
    miou, _, _ = calculate_miou(gt_boundary_labels, pred_boundary_labels, n_class=n_class)
    return miou
...
def main(pred_ply, gt_ply):
    ...
    # 计算整体指标
    print("整体评估指标：")
    mIOU, class_ious, weights = calculate_miou(gt_labels_sorted, pred_labels_sorted, n_class)
    print(f"  mIOU: {mIOU:.4f}")
    
    # 计算background
    gt_background = (gt_labels_sorted == 0).astype(np.int64)
    pred_background = (pred_labels_sorted == 0).astype(np.int64)
    gt_background_torch = torch.from_numpy(gt_background)
    pred_background_onehot = torch.from_numpy(np.eye(2)[pred_background]).float()
    _, _, _, _, background, _ = cal_metric(gt_labels=gt_background_torch, pred_labels=pred_background_onehot, target_class=1)
    print(f"  background: {background:.4f}")
    
    # 计算boundary
    boundary = calculate_boundary_miou(gt_xyzs[gt_order], gt_labels_sorted, pred_labels_sorted, n_class, k=8)
    ...
    result_lines.append(f"  mIOU: {mIOU:.4f}")
    result_lines.append(f"  boundary: {boundary:.4f}")
    result_lines.append(f"  background: {background:.4f}")
    result_lines.append("")
