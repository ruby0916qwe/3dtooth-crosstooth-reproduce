...
class UpToothSeg(nn.Module):
    def __init__(self, num_features=6, num_classes=17, k=16):
        super(UpToothSeg, self).__init__()
        self.k = k
        self.transform_net = Transform_Net(num_features)
        ...
        self.conv11 = nn.Conv1d(128, num_classes, kernel_size=1, bias=True)

        self.edge_seg_head = nn.Conv1d(128, 2, kernel_size=1, bias=True)
    ...
    def forward(self, x):
        ...
        seg_result = self.conv11(x)                      # (batch_size, 256, num_points) -> (batch_size, seg_num_all, num_points)
...
if __name__ == '__main__':
    ...
    model = UpToothSeg(num_features=6, num_classes=17, k=16)
