...
class GraphTransformerBlock(nn.Module):
    def __init__(self, feature_dim, out_dim, K):
        ...
        self.sa = nn.MultiheadAttention(embed_dim=feature_dim, num_heads=1)
        self.K = K
...
class SemanticsPrediction(nn.Module):
    def __init__(self, feature_dim=512, output_channels=17):
...
class SGTNet(nn.Module):
    def __init__(self, k=16, pt_num=16000, in_channels=6, output_channels=17):
        ...
        self.bn4_c = nn.BatchNorm1d(256)
        self.conv1_c = nn.Sequential(nn.Conv2d(in_channels, 32, kernel_size=1, bias=False),
                                   self.bn1_c,
                                   nn.LeakyReLU(negative_slope=0.2))


        self.conv2_c = nn.Sequential(nn.Conv2d(32*2, 64, kernel_size=1, bias=False),
                                   self.bn2_c,
                                   nn.LeakyReLU(negative_slope=0.2))
        ...
        self.conv4_c = nn.Sequential(nn.Conv1d(224, 256, kernel_size=1, bias=False),
                                     self.bn4_c,
                                     nn.LeakyReLU(negative_slope=0.2))

        self.graph_transformer_block1 = GraphTransformerBlock(feature_dim=3, out_dim=32, K=self.k)
        self.graph_transformer_block2 = GraphTransformerBlock(feature_dim=32, out_dim=64, K=self.k)
        self.graph_transformer_block3 = GraphTransformerBlock(feature_dim=64, out_dim=128, K=self.k)
        self.FTM_c1 = STNkd(k=3)
        ''' normal stream '''
        self.bn1_n = nn.BatchNorm2d(32)
        self.bn2_n = nn.BatchNorm2d(64)
        self.bn3_n = nn.BatchNorm2d(128)
        self.bn4_n = nn.BatchNorm1d(256)
        self.conv1_n = nn.Sequential(nn.Conv2d(in_channels, 32, kernel_size=1, bias=False),
                                     self.bn1_n,
                                     nn.LeakyReLU(negative_slope=0.2))


        self.conv2_n = nn.Sequential(nn.Conv2d(32*2, 64, kernel_size=1, bias=False),
                                     self.bn2_n,
                                     nn.LeakyReLU(negative_slope=0.2))
        ...
        self.conv4_n = nn.Sequential(nn.Conv1d(224, 256, kernel_size=1, bias=False),
                                     self.bn4_n,
                                     nn.LeakyReLU(negative_slope=0.2))
        self.global_graph_attention1 = nn.MultiheadAttention(embed_dim=256, num_heads=4)
        self.global_graph_attention2 = nn.MultiheadAttention(embed_dim=256, num_heads=4)
        self.FTM_n1 = STNkd(k=3)

        self.fa = nn.Sequential(nn.Conv1d(512, 512, kernel_size=1, bias=False),
                                nn.BatchNorm1d(512),
                                nn.LeakyReLU(0.2))

        ''' feature fusion '''
        self.pred1 = nn.Sequential(nn.Conv1d(512, 128, kernel_size=1, bias=False),
                                   nn.BatchNorm1d(128),
                                   nn.LeakyReLU(negative_slope=0.2))
        self.pred4 = nn.Sequential(nn.Conv1d(128, output_channels, kernel_size=1, bias=False))

        # 边界分割头
        self.edge_seg_head = nn.Sequential(nn.Conv1d(128, 2, kernel_size=1, bias=False))
        ...
        self.semantics_prediction0 = SemanticsPrediction(feature_dim=6)
        self.semantics_prediction1 = SemanticsPrediction(feature_dim=64)
        self.semantics_prediction2 = SemanticsPrediction(feature_dim=128)
...
if __name__ == "__main__":
    ...
    x = torch.rand(1, 6, 16000)
    x = x.cuda()
    model = SGTNet(in_channels=6, output_channels=17, k=3)
    model = model.cuda()
    seg_result, edge_seg_result = model(x)
