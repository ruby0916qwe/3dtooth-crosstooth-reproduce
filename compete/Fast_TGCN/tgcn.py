...
def pairwise_distance(points):
    ...
    points_transpose = points.permute(0, 2, 1)  # B, N, C

    # 广播机制，计算距离的平方和
    dist_sq = (
        points_transpose.unsqueeze(2) - points_transpose.unsqueeze(1)
    ).pow(2).sum(dim=3)  # B, N, N
...
class TGCN(nn.Module):
    def __init__(self, in_channels=6, output_channels=17):
        ...
        self.conv1_c = nn.Sequential(nn.Conv1d(in_channels // 2, 64, kernel_size=1, bias=False),
                                   self.bn1_c,
                                   nn.LeakyReLU(negative_slope=0.2),
                                   nn.Conv1d(64, 64, kernel_size=1, bias=False))


        self.conv1_n = nn.Sequential(nn.Conv1d(in_channels // 2, 64, kernel_size=1, bias=False),
                                   self.bn1_c,
                                   nn.LeakyReLU(negative_slope=0.2),
                                   nn.Conv1d(64, 64, kernel_size=1, bias=False))
        ...
        # 边界分割头
        self.edge_seg_head = nn.Sequential(nn.Linear(128, 2, bias=False))
    ...
    def forward(self, x):
        ...
        # 计算邻接矩阵
        distances = pairwise_distance(coor)

        # 这个邻居的定义是手动的
        adj = torch.where(distances >= 0.1, 1., 0.)
...
if __name__ == '__main__':
    model = TGCN(in_channels=6, output_channels=17)
